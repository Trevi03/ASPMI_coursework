function [y_hat, error, w, eps] = GNGD(x_input,z_input,mu,rho,epsilon,order)
...
%            evol_weights = evolution of the adaptive weights in time, (Nw + 1) × N matrix
    ...
    % delay = 1;
    eps = (1/mu)*ones(length(x_input)+1,1);
    eps(1) = epsilon;
    beta=1;
    ...
    for i = 1 : N 
        ...
        w(:, i + 1) = w(:,i) + ((beta*error(i))/(eps(i) +(delayed_x(:,i))'*delayed_x(:,i)))*delayed_x(:,i);
        if i>1
            eps(i+1) = eps(i) - (rho*mu)*((error(i)*error(i-1)*(delayed_x(:,i)')*delayed_x(:,i-1))/((eps(i-1)+(delayed_x(:,i-1)')*delayed_x(:,i-1)))^2);
        end
    end 
    w = w(:, 2:end);
    eps = eps(:,2:end);
